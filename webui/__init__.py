...
from manager.ProcessManager import TxchangecoindManager, WalletRPCManager
...
class BaseWebUI(QMainWindow):
    def __init__(self, html, app, hub, window_size, debug=False):
        ...
        self.icon = self._getQIcon('txchangecoin_icon_64.png')
...
class MainWebUI(BaseWebUI):
    def __init__(self, app, hub, debug):
        ...
        if sys.platform == 'darwin':
            tray_icon = 'txchangecoin_16x16_mac.png'
        elif sys.platform == "win32":
            tray_icon = 'txchangecoin_16x16.png'
        else:
            tray_icon = 'txchangecoin_32x32_ubuntu.png'
        ...
        self.txchangecoind_daemon_manager = None
        self.wallet_rpc_manager = None
        self.txchangecoind_daemon_pid = None
...
#         self.view.load(qt_core.QUrl(self.url))
    ...
    def start_deamon(self):
        #start txchangecoind daemon
        self.txchangecoind_daemon_manager = TxchangecoindManager(self.app.property("ResPath"),
        ...
        self.txchangecoind_daemon_manager.start()
        self.txchangecoind_daemon_pid = self.txchangecoind_daemon_manager.get_pid()
    ...
    def run_wallet_rpc(self, log_level=2):
        ...
        while True:
            ...
            txchangecoind_info = self.daemon_rpc_request.get_info()
            if txchangecoind_info['status'] == "OK":
    ...
    def _update_daemon_status(self):
        ...
        if txchangecoind_info['status'] == "OK":
            status = "Connected"
            self.current_height = int(txchangecoind_info['height'])
            target_height = int(txchangecoind_info['target_height'])
            ...
            if not self.daemon_version:
                self.daemon_version = txchangecoind_info['version']
        else:
            status = txchangecoind_info['status']
        ...
        sync_status = "Disconnected" if txchangecoind_info['status'] != "OK" else "Synchronizing..."
        if txchangecoind_info['status'] == "OK" and self.current_height == self.target_height:
...
#             print(json.dumps(wallet_info, indent=4))
    ...
    def about(self):
        ...
        QMessageBox.about(self, "About", \
            u"%s %s <br><br>Copyright© 2019 - Txchangecoin Projects" % \
    ...
    def _handleAboutToQuit(self):
        ...
        self.stop_update_wallet_info_timer()
        if self.txchangecoind_daemon_manager is not None:
            self.txchangecoind_daemon_manager.stop()
        if self.wallet_rpc_manager is not None:
