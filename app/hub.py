...
class Hub(QObject):
    ...
    def restore_deterministic_wallet(self, mnemonic_seed, restore_height=0, seed_offset_passphrase=u''):
        ...
        if language_name is None:
            QMessageBox.critical(self.new_wallet_ui, \
                    ...
                    NOTE: Txchangecoin mnemonic seed must be a list of 26 (valid) words""")
            return
    ...
    def send_tx(self, amount, address, payment_id, priority, mixin, tx_desc, save_address, sweep_all):
        if not payment_id and not address.startswith("TXX"):
    ...
    def restart_daemon(self):
        self.app_process_events(0.1)
        self.ui.txchangecoind_daemon_manager.stop()
        self.ui.start_deamon()
        while True:
            txchangecoind_info = self.ui.daemon_rpc_request.get_info()
    ...
    def view_daemon_log(self):
        log_file = os.path.join(DATA_DIR, 'logs', "txchangecoind.log")
